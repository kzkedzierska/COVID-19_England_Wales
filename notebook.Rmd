...
current_2020_spreadsheet <- "./data/referencetablesweek14202013042020165839.xlsx"
...
place_of_death <- 
  read_xlsx(current_2020_spreadsheet, 
            ...
            skip = 4)[1:6,2:9] %>%
  rename(cause = `...2`) %>%
  mutate_at(vars(-cause), as.numeric) %>%
  mutate(country = c(rep("England and Wales", 2), rep("England", 2), rep("Wales", 2))) %>%
  pivot_longer(names_to = "place", 
               values_to = "number_of_deaths",
               c(-cause, -country))
...
actual_vs_reported <-
  read_xlsx(current_2020_spreadsheet, 
            ...
            col_types = c("date", rep("numeric", 8)))[1:30, 1:9] %>%
  pivot_longer(names_to = "source",
               values_to = "number_of_deaths",
               -Date)

age_distribution <- 
  read_xlsx(current_2020_spreadsheet, sheet = sheets_2020[str_detect(sheets_2020, regex("covid-19 .* occurrences", ignore_case = TRUE))], 
...
As of `r format(data_up_to, '%b, %d')`, majority of the deaths are reported in hospitals. 
...
label_postions <- 
  ...
  mutate(fixed_position_cum = 2000 + 350 * 1:length(col_scheme),
         fixed_position_daily = 375 + 50 * 1:length(col_scheme))
...
Looking at the daily death rates it becomes clear that there is a significant lag in the data. And the drops in the registered deaths on the weekends (March 21&22, 28&29). 
...
Looking at the deaths, Males are more affected than Females. There were no deaths in the age groups not shown at the time. This on its own may pose more questions rather than provide answers. How this plot looks like for cases? How does the population structure in UK look like? But for now I don't even know where to look for the data.
...
age_distribution %>%
  ...
  scale_fill_npg() +
  scale_y_continuous(breaks = seq(-max_axis, max_axis, 50),
                     labels = abs(seq(-max_axis, max_axis, 50))) +
  labs(x = "Age groups",
